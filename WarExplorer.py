...
def actionWarExplorer():
    ...
    for ressource in dico['percepts_ressources']:
        broadcastMessageToAgentType(WarAgentType.WarBase,'food',[str(ressource.getDistance()),str(ressource.getAngle())])
    ...
    for message in dico['messages']:
        if message.getSenderType() == WarAgentType.WarBase and message.getMessage() == 'food' :

            food = Trigo.getPolarFromMessage(message)
            confirmFoodExistance = True

            if food['distance'] < distanceOfView() and Trigo.inView(getHeading(), angleOfView(), food['angle']) :
                inPerception = False

                for ressource in dico['percepts_ressources']:
                    if Trigo.roundCoordinates(Trigo.toCarthesian(food)) == Trigo.roundCoordinates(Trigo.toCarthesian({'distance': ressource.getDistance(), 'angle': ressource.getAngle()})):
                        inPerception = True
                        break

                if inPerception == False:
                    confirmFoodExistance = False
                    broadcastMessageToAgentType(WarAgentType.WarBase,'nofood',[str(food['distance']),str(food['angle'])])
...
class GoHomeState(object):
    ...
    def execute():
        ...
        if len(dico['percepts_alliesBase']) != 0 :
            ...
            for message in dico['messages'] :
                if(message.getSenderType() == WarAgentType.WarBase and message.getMessage() == 'base'):
